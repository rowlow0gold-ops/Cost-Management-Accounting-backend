...
import com.demo.cost.domain.*;
import com.demo.cost.repository.*;
...
public class DataBootstrap implements CommandLineRunner {
    ...
    private void seedUsers() {
        ...
        userRepo.saveAll(List.of(
            User.builder().email("admin@demo.com").password(hash).name("관리자")
                .role(Role.ADMIN).department(depts.get(0)).build(),
            User.builder().email("manager@demo.com").password(hash).name("본부장")
                .role(Role.MANAGER).department(depts.get(1)).build(),
            User.builder().email("user@demo.com").password(hash).name("일반사용자")
    ...
    private void seedAuditLogsFast() {
        String[] actors = {"admin@demo.com","manager@demo.com","user@demo.com"};
